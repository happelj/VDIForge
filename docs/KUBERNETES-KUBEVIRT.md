...
Phase 7 uses the same KubeVirt/CDI/storage foundation to clone promoted source PVCs into per-desktop DataVolumes and to create KubeVirt `VirtualMachine` resources from the provisioner. Phase 8 uses `ubuntu-devops:1.1.0` for remote desktop validation, Phase 9 uses `ubuntu-devops:1.2.0` for portal-launched remote desktop validation, and Phase 14 validates all final catalog source PVCs while keeping DevOps `1.2.0` as the primary graphical browser proof. RDP access remains restricted to the Guacamole path. See [FastAPI VDI Control Plane](API-CONTROL-PLANE.md), [Remote Desktop Delivery](REMOTE-DESKTOP.md), and [React Self-Service Portal](WEB-PORTAL.md).
