...
| SEC-017 | Phase 12 shall document the platform secret inventory, storage locations, readers, persistence, rotation, deletion behavior, and Git exposure status without recording real secret values. | Security hardening documentation and secret scan. |
| SEC-018 | Kubernetes ServiceAccount access to Secrets shall be reviewed and validated so frontend pods cannot read Secrets and VDIForge API/provisioner access is limited to documented needs. | `kubectl auth can-i` positive and negative tests. |
| SEC-019 | Keycloak shall retain PKCE, disable insecure browser-client flows, restrict redirect/origin policy, and enable practical local-lab brute-force protection. | Realm JSON review, admin CLI validation, and OIDC regression tests. |
| SEC-020 | Frontend token/session behavior shall avoid client secrets, avoid token logging, clear local session state on logout, and document browser storage tradeoffs. | Static frontend review and portal regression tests. |
| SEC-021 | Browser-facing VDIForge endpoints shall emit appropriate security headers, with documented exceptions for Keycloak, Guacamole, and Grafana compatibility. | Curl header validation. |
| SEC-022 | FastAPI CORS shall allow only expected portal origins for authenticated browser access and shall not use wildcard origins. | CORS positive and negative tests. |
| SEC-023 | High-impact desktop lifecycle and connect operations shall have lab-appropriate abuse resistance through authentication, quota, idempotency, and rate limiting. | Backend unit tests and live API security tests. |
| SEC-024 | User-controlled identifiers, display names, resource profiles, idempotency keys, and path IDs shall be validated before use in Kubernetes, SQL, URL, or label contexts. | Backend input-validation tests and live negative tests. |
| SEC-025 | VDIForge-owned platform containers shall use restrictive security contexts where practical, and exceptions shall be documented. | Helm render review and live pod-spec validation. |
| SEC-026 | Phase 12 shall perform backend dependency, frontend dependency, and custom container image vulnerability scans, documenting or remediating critical/high findings. | `scripts/phase12-dependency-scan.sh` output. |
| SEC-027 | The golden-image pipeline shall be reviewed for build-secret cleanup, fixed-password avoidance, intentional xrdp configuration, and source checksum discipline. | Golden-image hardening review and Phase 6 regression validation. |
| SEC-028 | NetworkPolicies shall be reviewed and validated with positive and negative connectivity tests across platform, identity, monitoring, Guacamole, database, and VDI desktop paths. | `scripts/phase12-networkpolicy-test.sh` and live validation. |
| SEC-029 | Audit read/export APIs shall be admin-only and audit records shall not contain passwords, access tokens, refresh tokens, xrdp credentials, or Guacamole secrets. | API security tests and audit export scan. |
| SEC-030 | Audit records shall include tamper-evident hash-chain metadata while documenting local database limitations. | Migration/model review and audit API/export validation. |
| SEC-031 | Prometheus metrics and Grafana dashboards shall avoid sensitive or unbounded labels such as usernames, token subjects, request IDs, desktop IDs, raw URLs, or credentials. | Metrics scrape review and dashboard validation. |
| SEC-032 | Phase 12 validation shall produce explicit PASS/FAIL results and verify the browser VDI workflow remains functional after hardening. | `scripts/validate-phase12.ps1` and `scripts/validate-phase12-live.sh`. |
...
| Requirement | Implementation reference | Test or evidence | Status | Notes |
| --- | --- | --- | --- | --- |
| `NFR-006` | [helm/vdiforge/values-phase12-local.yaml](../helm/vdiforge/values-phase12-local.yaml), [helm/vdiforge/templates/securityheaders.yaml](../helm/vdiforge/templates/securityheaders.yaml) | Helm lint/template and live upgrade validation. | PASS | Phase 12 hardening is applied through Helm values and templates. |
| `NFR-008` | [docs/ADR/0022-security-and-audit-hardening.md](ADR/0022-security-and-audit-hardening.md) | Architecture review. | PASS | Phase 12 avoids Vault clusters, SIEM deployment, service mesh, WAF, and distributed rate limiting. |
| `FR-002` | [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Missing-token and tampered-token live tests. | PASS | Protected API paths still require valid bearer tokens. |
| `FR-003` | [backend/app/auth/jwt.py](../backend/app/auth/jwt.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | OIDC/JWT regression tests. | PASS | Token signature, issuer, audience, and expiration validation remains intact. |
| `FR-018` | [backend/app/services/desktops.py](../backend/app/services/desktops.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Cross-user connect denial. | PASS | Remote connection creation remains owner/admin enforced. |
| `FR-019` | [backend/app/services/remote_access.py](../backend/app/services/remote_access.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | API response and audit export scan. | PASS | Reusable xrdp/Guacamole credentials are not returned to the frontend. |
| `FR-024` | [backend/app/audit/service.py](../backend/app/audit/service.py), [backend/alembic/versions/0002_phase12_audit_integrity.py](../backend/alembic/versions/0002_phase12_audit_integrity.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Audit persistence, export, redaction, and hash metadata checks. | PASS | Audit events remain separate from application logs and gain hash-chain metadata. |
| `SEC-001` | [backend/app/services/desktops.py](../backend/app/services/desktops.py), [backend/app/api/routes.py](../backend/app/api/routes.py) | Backend authorization and live negative tests. | PASS | Authorization remains server-side after hardening. |
| `SEC-005` | [scripts/phase12-networkpolicy-test.sh](../scripts/phase12-networkpolicy-test.sh), [scripts/validate-phase12-live.sh](../scripts/validate-phase12-live.sh) | Live NetworkPolicy positive and negative tests. | PASS | Unauthorized database, Keycloak, Guacamole, and RDP paths are denied. |
| `SEC-008` | [.gitignore](../.gitignore), [scripts/validate-phase12.ps1](../scripts/validate-phase12.ps1), [scripts/phase12-inventory.sh](../scripts/phase12-inventory.sh), [docs/SECURITY-HARDENING.md](SECURITY-HARDENING.md) | Secret scan and secret metadata inventory. | PASS | No real secrets, kubeconfigs, local env files, private keys, or generated credentials are committed. |
| `SEC-009` | [backend/app/security/redaction.py](../backend/app/security/redaction.py), [backend/app/observability/logging.py](../backend/app/observability/logging.py), [backend/tests/test_api_authorization.py](../backend/tests/test_api_authorization.py) | Log redaction tests. | PASS | Sensitive header, token, password, and credential terms are redacted from application logs. |
| `SEC-014` | [backend/app/schemas/api.py](../backend/app/schemas/api.py), [backend/app/api/routes.py](../backend/app/api/routes.py), [backend/tests/test_api_authorization.py](../backend/tests/test_api_authorization.py) | Backend input-validation tests. | PASS | Unsafe image IDs, display names, UUIDs, profiles, and idempotency keys are rejected. |
| `SEC-015` | [backend/app/security/headers.py](../backend/app/security/headers.py), [helm/vdiforge/templates/securityheaders.yaml](../helm/vdiforge/templates/securityheaders.yaml), [scripts/phase12-security-headers-test.sh](../scripts/phase12-security-headers-test.sh) | Trusted curl header validation. | PASS | Browser endpoints keep HTTPS and add practical security headers. |
| `SEC-016` | [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Guessed-ID and cross-user connect tests. | PASS | Guacamole handoff remains protected by API authorization. |
| `SEC-017` | [docs/SECURITY-HARDENING.md](SECURITY-HARDENING.md), [scripts/phase12-inventory.sh](../scripts/phase12-inventory.sh) | Documentation review and live Secret metadata inventory. | PASS | Secret categories, storage, readers, persistence, rotation, deletion, and Git exposure are documented. |
| `SEC-018` | [scripts/phase12-rbac-test.sh](../scripts/phase12-rbac-test.sh), [helm/vdiforge/templates/rbac.yaml](../helm/vdiforge/templates/rbac.yaml) | `kubectl auth can-i` positive and negative tests. | PASS | Frontend has no Secret access; provisioner is namespace scoped. |
| `SEC-019` | [keycloak/realm/vdiforge-realm.json](../keycloak/realm/vdiforge-realm.json), [scripts/phase12-keycloak-hardening.sh](../scripts/phase12-keycloak-hardening.sh) | Realm JSON and live Keycloak admin CLI validation. | PASS | PKCE, redirect/origin restrictions, disabled implicit flow, and brute-force protection are validated. |
| `SEC-020` | [frontend/src/auth/oidc.ts](../frontend/src/auth/oidc.ts), [docs/WEB-PORTAL.md](WEB-PORTAL.md), [docs/SECURITY-HARDENING.md](SECURITY-HARDENING.md) | Static frontend session review. | PASS | The browser client uses `sessionStorage`, no client secret, and documented logout/session behavior. |
| `SEC-021` | [backend/app/security/headers.py](../backend/app/security/headers.py), [helm/vdiforge/templates/securityheaders.yaml](../helm/vdiforge/templates/securityheaders.yaml), [monitoring/kube-prometheus-stack-values-local.yaml](../monitoring/kube-prometheus-stack-values-local.yaml) | Header curl checks across VDIForge hosts. | PASS | Headers are tailored for API, portal, Keycloak, Guacamole, and Grafana compatibility. |
| `SEC-022` | [backend/app/config/settings.py](../backend/app/config/settings.py), [scripts/phase12-security-headers-test.sh](../scripts/phase12-security-headers-test.sh) | Allowed-origin and unrelated-origin CORS checks. | PASS | Authenticated API CORS does not use wildcard origins. |
| `SEC-023` | [backend/app/security/rate_limit.py](../backend/app/security/rate_limit.py), [backend/app/api/routes.py](../backend/app/api/routes.py), [backend/tests/test_api_authorization.py](../backend/tests/test_api_authorization.py) | Backend rate-limit tests and live security validation. | PASS | Desktop mutation/connect operations have lab rate limits plus quota and idempotency. |
| `SEC-024` | [backend/app/schemas/api.py](../backend/app/schemas/api.py), [backend/app/api/routes.py](../backend/app/api/routes.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Malformed and unsafe-input negative tests. | PASS | User input cannot directly create unsafe Kubernetes names, URLs, or query fragments. |
| `SEC-025` | [helm/vdiforge/templates/api.yaml](../helm/vdiforge/templates/api.yaml), [helm/vdiforge/templates/frontend.yaml](../helm/vdiforge/templates/frontend.yaml), [helm/vdiforge/templates/provisioner.yaml](../helm/vdiforge/templates/provisioner.yaml), [docs/SECURITY-HARDENING.md](SECURITY-HARDENING.md) | Helm render and pod-spec review. | PASS | VDIForge-owned containers keep restrictive security contexts where practical. |
| `SEC-026` | [scripts/phase12-dependency-scan.sh](../scripts/phase12-dependency-scan.sh) | Backend, frontend, Trivy image, and optional config scans. | PASS | Critical/high findings are surfaced for remediation or accepted-risk documentation. |
| `SEC-027` | [docs/SECURITY-HARDENING.md](SECURITY-HARDENING.md), [docs/GOLDEN-IMAGES.md](GOLDEN-IMAGES.md), [scripts/validate-phase6.ps1](../scripts/validate-phase6.ps1) | Image hardening review and Phase 6 regression. | PASS | Golden images avoid committed/build secrets and keep xrdp configuration intentional. |
| `SEC-028` | [scripts/phase12-networkpolicy-test.sh](../scripts/phase12-networkpolicy-test.sh), [scripts/validate-phase12-live.sh](../scripts/validate-phase12-live.sh) | Live NetworkPolicy validation with disposable pods and desktop. | PASS | Intended paths work while unauthorized paths are denied. |
| `SEC-029` | [backend/app/api/routes.py](../backend/app/api/routes.py), [backend/app/audit/service.py](../backend/app/audit/service.py), [scripts/phase12-api-security-test.py](../scripts/phase12-api-security-test.py) | Admin/non-admin audit tests and audit export scan. | PASS | Audit read/export is admin-only and redacts secrets. |
| `SEC-030` | [backend/app/security/audit_integrity.py](../backend/app/security/audit_integrity.py), [backend/app/audit/service.py](../backend/app/audit/service.py), [backend/alembic/versions/0002_phase12_audit_integrity.py](../backend/alembic/versions/0002_phase12_audit_integrity.py) | Backend tests and live audit hash validation. | PASS | Audit events include `previous_event_hash` and `event_hash`. |
| `SEC-031` | [backend/app/observability/metrics.py](../backend/app/observability/metrics.py), [monitoring/grafana/vdiforge-overview.json](../monitoring/grafana/vdiforge-overview.json), [scripts/validate-phase12-live.sh](../scripts/validate-phase12-live.sh) | Metrics scrape and dashboard review. | PASS | Metrics avoid usernames, subjects, desktop IDs, request IDs, tokens, and credentials. |
